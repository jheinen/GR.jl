...
if isfile(depsfile)
    try
        ...
        @debug "Error while trying to include GR.depsfile:" depsfile err
    end
else
    @debug "GR.depsfile does not exist" depsfile
end

if os == :Windows
    const libGR = Ref("libGR.dll")
    const libGR3 = Ref("libGR3.dll")
    const libGRM = Ref("libGRM.dll")
elseif os == :Darwin
    const libGR = Ref("libGR.dylib")
    const libGR3 = Ref("libGR3.dylib")
    const libGRM = Ref("libGRM.dylib")
else
    const libGR = Ref("libGR.so")
    const libGR3 = Ref("libGR3.so")
    const libGRM = Ref("libGRM.so")
end
    ...
    if grdir == None
        ...
        if !isdir(grdir)
            if attempt_to_rebuild[]
                ...
                touch(pathof(GR))
                @info """Rebuilding GR succeeded.
                GR may need to be precompiled once more in the next Julia
                session, but is ready to use now."""
            else
