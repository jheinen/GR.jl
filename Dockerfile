...
RUN arch=$(arch | sed 's/arm64/aarch64/') && \
    ...
    wget https://julialang-s3.julialang.org/bin/linux/${dir}/1.11/julia-1.11.2-linux-${arch}.tar.gz && \
    tar -xzvf julia-1.11.2-linux-${arch}.tar.gz && ls && \
    cp -R julia-1.11.2/* /usr && \
    rm -rf ${HOME}/julia-1.11.2*
